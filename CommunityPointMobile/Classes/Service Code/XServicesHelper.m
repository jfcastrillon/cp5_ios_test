...
- (void)loadResourceDetails: (NSDecimalNumber*) resourceId{
    XSResourceDetailsOperation *op = [[XSResourceDetailsOperation alloc] initWithResourceId: [resourceId intValue] andSearchHistoryId:[lastSearchResultSet searchHistoryId]];
    op.delegate = self;
    [operationQueue addOperation: op];
    [op release];
}
...
- (void) operationDidComplete: (XSResponse*) response {
    ...
    } else if([[response tag] isEqualToString: @"resources.pullWithServiceDetails"]) {
...
- (void) operationDidFailWithError: (NSDictionary*) errorInfo {
    ...
    } else if ([tag isEqualToString:@"resources.pullWithServiceDetails"]) {
